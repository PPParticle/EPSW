...
void SpMSpM(ELLPACK_P M1, ELLPACK_P M2, ELLPACK_P M3){
    ...
    unsigned int col = 0;
    unsigned int *tmp_nnz = new unsigned int [M2._rows];
    unsigned int *tmp_row = new unsigned int [M2._rows];
    unsigned int **tmp_col = new unsigned int* [M2._rows];
    ...
    for(unsigned int i=0; i<M3._rows; i++){
        ...
        unsigned int *tmp_c_col = new unsigned int [col]{0};
        M3._rowidx[i] = i;
    ...
    for(unsigned int i=0; i<M1._rows; i++){
        ...
        unsigned int b_row = M2._rowidx[i];
        
        for(unsigned int j=0; j<M1._nnz_num[i]; j++){
            
            Val tmp_a_v = M1._rowPtr[i][j];
            unsigned int tmp_a_row = M1._rowidx[i];
            unsigned int tmp_a_col = M1._coordPtr[i][j];
            
            Val *tmp_b_v = M2._rowPtr[tmp_a_col];
            unsigned int *tmp_b_col = M2._coordPtr[tmp_a_col];

            Val *tmp_c_v = M3._rowPtr[tmp_a_row];
            unsigned int *tmp_c_col = M3._coordPtr[tmp_a_row];

            for(unsigned int l=M2._nnz_num[tmp_a_col]; l>0; l-=vl){
                vl = vsetvl_e32m8(l);
...
void scalar_normal(Val *a, const unsigned int ai, const unsigned int aj,
...
int main(int argc, char **argv){
    ...
    int dim = atoi(argv[1]);
    unsigned int d = static_cast<unsigned int>(dim);
