...
void print_ELLPACK_P(ELLPACK_P M)
{
    ...
    for (unsigned int i = 0; i < M._rows; i++)
    {
        if (M._nnz_num[i] == 0)
            continue;
        for (unsigned int j = 0; j < M._nnz_num[i]; j++)
...
void sddmm_ELLP(unsigned int *c, const int ci, const int cj,
...
void initiation(float *a, unsigned int ai, unsigned int aj,
                float *b, unsigned int bi, unsigned int bj,
                unsigned int *c, unsigned int ci, unsigned int cj)
{
    for (int i = 0; i < ai; i++)
    {
        for (int j = 0; j < aj; j++)
...
void matrixtoELLPACK(int *c, unsigned int ci, unsigned int cj, int **c_new, int *size, int **c_d)
{
...
int main(int argc, char *argv[])
{
    ...
    unsigned int *c = new unsigned int[n * n];
