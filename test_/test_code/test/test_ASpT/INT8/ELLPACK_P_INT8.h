...
public:
    ...
    void init(Idx rows, Val **rowPtr, Idx **coordPtr, Idx *nnz_num, Idx *nnz_old_num,
    ...
    void print_nnz(){
        for(int i=0; i<this->_rows; i++){
            cout << (int32_t)this->_nnz_num[i] << " ";
        }
        cout << endl;
    }
    void print_nnz_old(){
        for(int i=0; i<this->_rows; i++){
            cout << (int32_t)this->_nnz_old_num[i] << " ";
        }
        cout << endl;
    }
    void print_ELLPACK_P()
    {
        ...
        for (Idx i = 0; i < this->_rows; i++)
        {
            for (Idx j = 0; j < this->_nnz_num[i]; j++)
                // cout << this->_rowPtr[i][j] << " ";
                printf("%5.2d ", (int32_t)this->_rowPtr[i][j]);
            cout << "         ";
            for (Idx j = 0; j < this->_nnz_num[i]; j++)
                // cout << this->_coordPtr[i][j] << " ";
                printf("%5d ", (int32_t)this->_coordPtr[i][j]);
            cout << endl;
        }
    }
    void print_rowidx(){
        for(int i=0; i<this->_rows; i++){
            cout << (int32_t)this->_rowidx[i] << " ";
        }
        cout << endl;
    }
    void print_rowPtr(){
    ...
    Idx merge(const Idx *src1, const Idx n1, 
            const Idx *src2, const Idx n2,  
            Idx *dst)
    {
        size_t vl, k, num = 0;
        ...
        {
            vl = vsetvl_e32m8(k);
            vuint32m8_t v1 = vle32_v_u32m8(src1 + n1 - k, vl);
        ...
        {
            vl = vsetvl_e32m8(k);
            vuint32m8_t v1 = vle32_v_u32m8(src2 + n2 - k, vl);
            vse32_v_u32m8(dst, v1, vl);
            dst += vl;
        }
        num += n2;
    ...
    void radix_sort(const Idx *src, Idx *dst, Idx *idx, const Idx n, Idx logic)
    {
        if (n > 1)
        {
            ...
            {
                vl = vsetvl_e32m8(k);
                vuint32m8_t vtmp = vle32_v_u32m8(src + n - k, vl);
                vuint32m8_t vidx = vle32_v_u32m8(idx + n - k, vl);
