...
Idx merge(const Idx *src1, const Idx n1, const Idx *src2, const Idx n2, Idx *dst)
    {
        ...
        for (; k > 0; k -= vl)
        {
            vl = vsetvli(k, RVV_E8, RVV_M8);
            uint32xm8_t v1 = vlev_uint8xm8(src1, vl);
            vsev_uint8xm8(dst, v1, vl);
            src1 += vl;
            dst += vl;
        }
        num += n1;
        ...
        {
            vl = vsetvli(k, RVV_E8, RVV_M8);
            uint8xm8_t v1 = vlev_uint8xm8(src2, vl);
            vsev_uint8xm8(dst, v1, vl);
            src2 += vl;
            dst += vl;
        }
...
void radix_sort(const Idx*src, Idx*dst, Idx*idx, const Idxn, Idxlogic)
    {
        if (n > 1)
        {
            size_t vl;
