    ...
    for(unsigned int i=0; i<M1._rows; i++){        
        for(unsigned int j=0; j<M1._nnz_num[i]; j++){
            ...
            for(unsigned int l=0; l<M2._nnz_num[a_col]; l+=vl){
				vc = vfmvvf_float32xm8(0.0, vsetvli_max(RVV_E32, RVV_M8));
                vl = vsetvli(M2._nnz_num[a_col]-l, RVV_E32, RVV_M8);
                vb = vlev_float32xm8(M2._rowPtr[a_col]+l, vl);
                vbidx = vlev_uint32xm8(M2._coordPtr[a_col]+l, vl);
				
                vcidx = vmvvv_uint32xm8(vbidx, vl);
                vcidx = vsllvx_uint32xm8(vbidx, 2, vl);
				
                vc = vlxev_float32xm8(M3._rowPtr[i], vcidx, vl);			
                vc = vfmaccvf_float32xm8(vc, a_v, vb, vl);  
                vsxev_uint32xm8(M3._coordPtr[i], vcidx, vbidx, vl); // base, index, value, vl
				vsxev_float32xm8(M3._rowPtr[i], vcidx, vc, vl);
            }
        }
    }
...
#define VALIDATE
	...
	// cout << EPSW_csr_v[nr] << endl;
	// cout << sca_csr_v[nr] << endl;
	...
	// fprintf(fout, "\n");
	for(int i=0; i<cnt; i++){
		if(sca_csr_ev[i]!=vec_csr_ev[i]){
			cout << "wrong" << endl;
			break;	
		}
		// cout << sca_csr_ev[i] << " " << vec_csr_ev[i]<< endl;
	}
