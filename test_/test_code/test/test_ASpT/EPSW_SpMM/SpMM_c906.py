...
data = pd.read_excel('c906_spmm_11.28.xlsx')
...
ell = data.iloc[0:parameter,8].values #epsw0_m2
epsw_1 = data.iloc[0:parameter,10].values #epsw1_m2
epsw_2 = data.iloc[0:parameter,12].values #epsw2_m2
...
# fig, ax= plt.subplots()
plt.bar(kb, base, bar_width, color=color[0], label="Base", )
plt.bar(kb+bar_width, aspt, bar_width, color=color[1], label="ASpT")
plt.bar(kb+2*bar_width, csr, bar_width, color=color[2], label="CSR")
plt.bar(kb+3*bar_width, ell, bar_width, color=color[3], label="ELLPACK")
plt.bar(kb+4*bar_width, epsw_1, bar_width, color=color[4], label="EPSW_s")
plt.bar(kb+5*bar_width, epsw_2, bar_width, color=color[5], label="EPSW_ada")
ax = plt.gca()
...
plt.legend(bbox_to_anchor=(0.7, 0.82), ncol=3, prop = {'size':12})
...
plt.subplots_adjust(bottom=0.19, left=0, right=0.98)
