            	...
            	for(int k=0; k<stripes[i].size() && row+k<m; k++){
                	if(j<M._nnz_num[row+k]){
                    	...
                    	for(int l=0; l<n; l+=vl){
	                        vl=vsetvli(n-l, RVV_E32, RVV_M2);
                        	vb=vlev_float32xm2(vin+brd_idx*n+l, vl);
                        	vtmp=vlev_float32xm2(tmp_c+l, vl);
                        	vtmp=vfmaccvf_float32xm2(vtmp, brd, vb, vsetvli_max(RVV_E32, RVV_M2));
                        	vsev_float32xm2(tmp_c+l, vtmp, vl);
                    	}
                	}
            	}          
        	}
	...
	// M.print_rowidx();
    	...
    	for(int i=0; i<M._rows; i++){
        	for(int j=0; j<M._nnz_num[i]; j++){
				...
				float32xm2_t vb;
				float32xm2_t vtmp = vfmvsf_float32xm2(0.0, vsetvli_max(RVV_E32, RVV_M2));
            	for(int l=0; l<n; l+=vl){
	                vl=vsetvli(n-l, RVV_E32, RVV_M2);
                   	vb=vlev_float32xm2(vin+brd_idx*n+l, vl);
                  	vtmp=vlev_float32xm2(tmp_c+l, vl);
