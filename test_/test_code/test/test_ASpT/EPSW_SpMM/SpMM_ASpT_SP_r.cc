		...
		for (int row_panel=0; row_panel<nr/BH; row_panel++){
			...
			for(stride = 0; stride < mcsr_cnt[row_panel+1]-mcsr_cnt[row_panel]-1; stride++){
				for(int i=row_panel*BH; i<(row_panel+1)*BH; i++){	
					...
					int32xm2_t vidx, vidx_t;
					size_t vl = vsetvli_max(RVV_E32, RVV_M2);
					for(j=loc1; j<interm; j+=8){
						va = vlev_float32xm2(csr_ev+j, vl);
						vidx = vlev_int32xm2(csr_e+j, vl);
						vidx = vmulvx_int32xm2(vidx, sc, vl);
						for(int k=0; k<sc; k++){
							vans = vfmvvf_float32xm2(0.0, vl);
							vtmp = vfmvvf_float32xm2(0.0, vl);
							vidx_t = vaddvx_int32xm2(vidx, k, vl);
							vidx_t = vsllvx_int32xm2(vidx_t, 2, vl);
							vb = vlxev_float32xm2(vin, vidx_t, vl);
							vtmp = vfmulvv_float32xm2(va, vb, vl);
							vans = vfredsumvs_float32xm2(vtmp, vans, vl);
							vout_ASpT_vec[i*sc+k] += vfmvfs_float32xm2 (vans, 1);
						}
					}
