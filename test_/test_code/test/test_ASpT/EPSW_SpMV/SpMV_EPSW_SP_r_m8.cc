...
void mprocess()
{
	...
	for(int loop=0;loop<ITER;loop++) {
		for (int i = 0; i < M._rows; i++){   
			...
			size_t vl, vlmax = vsetvli_max(RVV_E32, RVV_M8);
			float32xm8_t va, vb, vtmp, vans;
			uint32xm8_t vcidx;
			vans = vfmvvf_float32xm8(0.0, 1);
			vtmp = vfmvvf_float32xm8(0.0, vlmax);
			
			for(int j=0; j<num; j+=vl){
				vl = vsetvli(num-j, RVV_E32, RVV_M8);
				va = vlev_float32xm8(M._rowPtr[i]+j, vl);
				vcidx = vlev_uint32xm8(M._coordPtr[i]+j, vl);
				vcidx = vsllvx_uint32xm8(vcidx, 2, vl);
				vb = vlxev_float32xm8(vin, vcidx, vl);
				vtmp = vfmaccvv_float32xm8(vtmp, va, vb, vlmax);
			}
			vans = vfredsumvs_float32xm8(vtmp, vans, vlmax);
			vout_EPSW[ridx] += vfmvfs_float32xm8(vans, 1);	
