			...
			for (int i=row_panel*BH; i<(row_panel+1)*BH; i++){
				...
				if(loc2-loc1 >= 8) {
						...
						for(j=loc1; j<interm; j+=8){
							...
							vb = vlxev_float32xm2(vin, vcidx, vl);
							vtmp = vfmaccvv_float32xm2(vtmp, vb, va, vl);
						}
						vans = vfredsumvs_float32xm2(vtmp, vans, vl);
						vout_ASpT[i] += vfmvfs_float32xm2(vans, 1);
