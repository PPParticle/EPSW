...
void radix_sort(const unsigned int *src, unsigned int *dst, unsigned int *idx, const unsigned int n, unsigned int logic)
    ...
    void nnz_statics(){
        float f13 = (float)_cols/3, f23 = (float)_cols*2/3;
        size_t cnt13 = 0, cnt23 = 0, cnt33 = 0;
        for(size_t x=0; x<_rows; x++){
            (float)_nnz_num[x] < f13 ? cnt13++: _nnz_num[x] < f23 ? cnt23++: cnt33++;
        }
        fprintf(stdout, "f13 = %f f23 = %f f33 = %f\n", (float)cnt13/(float)_rows, (float)cnt23/(float)_rows, (float)cnt33/(float)_rows);
    }
};
#endif