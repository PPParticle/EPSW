...
void mprocess()
{
	...
	for(int loop=0;loop<ITER;loop++) {
		...
		for (int i = 0; i < nr; i++){
			...
			for (int j = 0; j < itr; j++){
				va = vle32_v_f32m2(csr_ev+csr_v[i]+j*vl, vl);
				vcidx = vle32_v_u32m2((unsigned int*)csr_e+csr_v[i]+j*vl, vl);
				vcidx = vsll_vx_u32m2(vcidx, 2, vl);
				vb = vloxei32_v_f32m2(vin, vcidx, vl);
				vtmp = vfmacc_vv_f32m2(vtmp, va, vb, vsetvlmax_e32m2());
			}
			vans = vfredusum_vs_f32m2_f32m1(vans, vtmp, vans, vsetvlmax_e32m2());
