...
void mprocess()
{
	...
	for(int loop=0;loop<ITER;loop++) {
		...
		for (int i = 0; i < M._rows; i++){   
			...
			vfloat32m1_t vans;
			vuint32m2_t vcidx;
			vans = vfmv_v_f_f32m1(0.0, vsetvlmax_e32m2());
			vtmp = vfmv_v_f_f32m2(0.0, vsetvlmax_e32m2());
			for(int j=0; j<num; j+=vl){
				vl = vsetvl_e32m2(num-j);
				va = vle32_v_f32m2(M._rowPtr[i]+j, vl);
				vcidx = vle32_v_u32m2(M._coordPtr[i]+j, vl);
				vcidx = vsll_vx_u32m2(vcidx, 2, vl);
				vb = vloxei32_v_f32m2(vin, vcidx, vl);
				vtmp = vfmacc_vv_f32m2(vtmp, va, vb, vsetvlmax_e32m2());
			}
			vans = vfredosum_vs_f32m2_f32m1(vans, vtmp, vans, vsetvlmax_e32m2());
			vout_EPSW[ridx] += vfmv_f_s_f32m1_f32(vans);		
		}
	}
