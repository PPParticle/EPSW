    ...
    void ELLPACKinitWithCSR(const int* csr_v, const int* csr_e, const Val* csr_ev, 
        const unsigned int nr, const unsigned int nc, const unsigned int ne){
            ...
            for(int i=1; i<nr+1; i++){
                ...
                this->_nnz_num[i-1] = this->_nnz_old_num[i-1] = csr_v[i] - csr_v[i-1];
                this->_rowPtr[i-1] = new Val[this->_nnz_num[i-1]];
                this->_coordPtr[i-1] = new unsigned int [this->_nnz_num[i-1]];
