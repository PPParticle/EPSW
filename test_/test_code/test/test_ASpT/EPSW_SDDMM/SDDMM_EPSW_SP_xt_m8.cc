	...
	for(int loop=0;loop<ITER;loop++) {
		 ...
		 for (int i = 0; i < M._rows; i++){   
			...
			for(int j=0; j<num; j+=vl){
				...
				va = vle32_v_f32m8(M._rowPtr[i]+j, vl);
				vidx = vle32_v_u32m8(M._coordPtr[i]+j, vl);
				vidx_s = vsll_vx_u32m8(vidx, 2, vl);
				vo = vfmv_v_f_f32m8(0.0, vsetvlmax_e32m8());
				vo = vloxei32_v_f32m8(vout_EPSW+m*nc, vidx_s, vl);
				for(int k=0; k<sc; k++){
					vidx_t = vmul_vx_u32m8(vidx, sc, vl);
					vidx_t = vadd_vx_u32m8(vidx_t, k, vl);
					vidx_t = vsll_vx_u32m8(vidx_t, 2, vl);
					vc = vloxei32_v_f32m8(vin, vidx_t, vl);
					vo = vfmacc_vf_f32m8(vo, vout[m*sc+k], vc, vl);
				}
				va = vfmul_vv_f32m8(va, vo, vl);
				vsoxei32_v_f32m8(vout_EPSW+m*nc, vidx_s, va, vl);
