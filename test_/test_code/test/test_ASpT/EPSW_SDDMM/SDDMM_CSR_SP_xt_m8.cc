...
#define ITER 10
	...
	for (int loop = 0; loop < ITER; loop++){
		...
		for(int i=0; i<nr; i++){
			...
			vfloat32m8_t va, vb;
			vuint32m8_t vidx, vidx_t;
			vb = vfmv_v_f_f32m8(0.0, vl);

			for(j=loc1; j<interm; j+=32){
				vidx = vle32_v_u32m8((unsigned int *)(csr_e+j), vl);
				vidx = vmul_vx_u32m8(vidx, sc, vl);

				for(int k=0; k<sc; k++){
					vidx_t = vadd_vx_u32m8(vidx, k, vl);
					vidx_t = vsll_vx_u32m8(vidx_t, 2, vl);
					va = vloxei32_v_f32m8(vin, vidx_t, vl);
					vb = vle32_v_f32m8(vout_rvv+j, vl);
					vb = vfmacc_vf_f32m8(vb, vout[i*sc+k], va, vl);
					vse32_v_f32m8(vout_rvv+j, vb, vl);
				}
				for(int k=0; k<32; k++) 
