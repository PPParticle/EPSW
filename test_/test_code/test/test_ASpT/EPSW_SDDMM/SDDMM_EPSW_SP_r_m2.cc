...
void mprocess()
{
	...
	for(int loop=0;loop<ITER;loop++) {
			...
			size_t vl;
			float32xm2_t va, vc, vo;
			int32xm2_t vidx, vidx_t, vidx_s;
		 ...
		 for (int i = 0; i < M._rows; i++){   
			...
			for(int j=0; j<num; j+=vl){
				...
				vidx = vlev_int32xm2((int *)M._coordPtr[i]+j, vl);
				vidx_s = vsllvx_int32xm2(vidx, 2, vl);
				vo = vfmvvf_float32xm2(0.0, vsetvli_max(RVV_E32, RVV_M2));
				vo = vlxev_float32xm2(vout_EPSW+m*nc, vidx_s, vl);
				for(int k=0; k<sc; k++){
					vidx_t = vmulvx_int32xm2(vidx, sc, vl);
					vidx_t = vaddvx_int32xm2(vidx_t, k, vl);
					vidx_t = vsllvx_int32xm2(vidx_t, 2, vl);
					vc = vlxev_float32xm2(vin, vidx_t, vl);
					vo = vfmaccvf_float32xm2(vo, vout[m*sc+k], vc, vl);
				}
				va = vfmulvv_float32xm2(va, vo, vl);
				vsxev_float32xm2(vout_EPSW+m*nc, vidx_s, va, vl);
