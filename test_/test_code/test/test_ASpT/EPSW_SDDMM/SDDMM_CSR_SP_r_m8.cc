...
{
	double elapsed[3];
	FTYPE *vin, *vout, *vout_scalar, *vout_rvv;
...
#define ITER 10
	...
	{		
		...
		for(int i=0; i<nr; i++){
			...
			int interm = loc1 + (((loc2-loc1)>>5)<<5);
			int j;

			size_t vl=vsetvli_max(RVV_E32, RVV_M8);
			float32xm8_t va, vb;
			uint32xm8_t vidx, vidx_t;
			vb = vfmvvf_float32xm8(0.0, vl);

			for(j=loc1; j<interm; j+=32){
				vidx = vlev_uint32xm8((unsigned int *)csr_e+j, vl);
				vidx = vmulvx_int32xm8(vidx, sc, vl);
				for(int k=0; k<sc; k++){
					vb = vlev_float32xm8(vout_rvv+j, vl);
					vidx_t = vaddvx_uint32xm8(vidx, k, vl);
					vidx_t = vsllvx_uint32xm8(vidx_t, 2, vl);
					va = vlxev_float32xm8(vin, vidx_t, vl);
					vb = vfmaccvf_float32xm8(vb, vout[i*sc+k], va, vl);
					vsev_float32xm8(vout_rvv+j, vb, vl);
				}
				for(int k=0; k<32; k++) 
			...
			for(; j<loc2; j++){
				for(int k=0; k<sc; k++) 
               		vout_rvv[j] = vout_rvv[j] + vin[csr_e[j]*sc + k] * vout[i*sc + k];
				vout_rvv[j] *= csr_ev[j];
			}
		}
	}
...
#define VALIDATE
	...
	for (int i = 0; i < nr * sc; i++)
	{
		...
		if (diff < 0)
			diff *= -1;
		// if (p1!=0 && MAX(p1, p2) !=0 && diff / MAX(p1, p2) > 0.01)
		// {	
