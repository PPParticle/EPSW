        	...
        	for(int j=0; j<M._nnz_num[row]; j++){
            	for(int k=0; k<stripes[i].size() && row+k<m; k++){
                	if(j<M._nnz_num[row+k]){
						...
						vfloat32m2_t vans = vfmv_v_f_f32m2(0.0, vsetvlmax_e32m2());
						vfloat32m2_t vtmp = vfmv_v_f_f32m2(0.0, vsetvlmax_e32m2());
                    	for(int l=0; l<sc; l+=vl){
	                        vl=vsetvl_e32m2(sc-l);
							vfloat32m2_t va=vle32_v_f32m2(tmp_a, vl);
                        	vfloat32m2_t vb=vle32_v_f32m2(tmp_b, vl); 
							vtmp=vfmul_vv_f32m2(va, vb, vl);	
                        	vans = vfadd_vv_f32m2(vans, vtmp, vl);
						...
						tmp = vfredosum_vs_f32m2_f32m1(tmp, vans, tmp, vl);
	...
	// M.print_rowidx();
    	...
    	size_t vl;
		 ...
		 for (int i = 0; i < M._rows; i++){   
			...
			size_t vl;
			vfloat32m2_t va, vb;
			vuint32m2_t vidx, vidx_t, vidx_s;

			for(int j=0; j<num; j+=vl){
				vl = vsetvl_e32m2(num-j);
				vidx = vle32_v_u32m2(M._coordPtr[i]+j, vl);
				vidx_s = vsll_vx_u32m2(vidx, 2, vl);

				for(int k=0; k<sc; k++){
					vb = vfmv_v_f_f32m2(0.0, vsetvlmax_e32m2());
					vidx_t = vmul_vx_u32m2(vidx, sc, vl);
					vidx_t = vadd_vx_u32m2(vidx_t, k, vl);
					vidx_t = vsll_vx_u32m2(vidx_t, 2, vl);
					va = vloxei32_v_f32m2(vin, vidx_t, vl);
				
					vb = vloxei32_v_f32m2(vout_EPSW+m*nc, vidx_s, vl);
					vb = vfmacc_vf_f32m2(vb, vout[m*sc+k], va, vl);
					
					vsoxei32_v_f32m2(vout_EPSW+m*nc, vidx_s, vb, vl);
				}
			}
			for(int j=0; j<num; j++){
