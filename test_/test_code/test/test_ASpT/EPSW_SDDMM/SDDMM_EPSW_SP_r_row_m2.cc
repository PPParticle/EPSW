        	...
        	for(int j=0; j<M._nnz_num[row]; j++){
            	for(int k=0; k<stripes[i].size() && row+k<m; k++){
                	if(j<M._nnz_num[row+k]){
						...
						float32xm2_t vans = vfmvvf_float32xm2(0.0, vsetvli_max(RVV_E32, RVV_M2));
						float32xm2_t vtmp = vfmvvf_float32xm2(0.0, vsetvli_max(RVV_E32, RVV_M2));
                    	for(int l=0; l<sc; l+=vl){
	                        vl=vsetvli(sc-l, RVV_E32, RVV_M2);;
							float32xm2_t va=vlev_float32xm2(tmp_a, vl);
                        	float32xm2_t vb=vlev_float32xm2(tmp_b, vl); 
							vtmp=vfmulvv_float32xm2(va, vb, vl);	
                        	vans = vfaddvv_float32xm2(vans, vtmp, vl);
					
                        	tmp_a += vl;
                        	tmp_b += vl;
                    	}
						
						float32xm2_t tmp = vfmvvf_float32xm2(0.0, vsetvli_max(RVV_E32, RVV_M2));
						tmp =  vfredsumvs_float32xm2(vans, tmp, vl);
						vout_EPSW[brd_i*nc + brd_j] +=  vfmvfs_float32xm2(tmp, 1);
	...
	// M.print_rowidx();
    	...
    	size_t vl;
		 ...
		 for (int i = 0; i < M._rows; i++){   
			...
			size_t vl;
			float32xm2_t va, vb;
			uint32xm2_t vidx, vidx_t, vidx_s;

			for(int j=0; j<num; j+=vl){
				vl = vsetvli(num-j, RVV_E32, RVV_M2);
				vidx = vlev_uint32xm2(M._coordPtr[i]+j, vl);
				vidx_s = vsllvx_uint32xm2(vidx, 2, vl);

				for(int k=0; k<sc; k++){
					vb = vfmvvf_float32xm2(0.0, vsetvli_max(RVV_E32, RVV_M2));
					vidx_t = vmulvx_int32xm2(vidx, sc, vl);
					vidx_t = vaddvx_uint32xm2(vidx_t, k, vl);
					vidx_t = vsllvx_uint32xm2(vidx_t, 2, vl);
					va = vlxev_float32xm2(vin, vidx_t, vl);
				
					vb = vlxev_float32xm2(vout_EPSW+m*nc, vidx_s, vl);
					vb = vfmaccvf_float32xm2(vb, vout[m*sc+k], va, vl);
					
					vsxev_float32xm2(vout_EPSW+m*nc, vidx_s, vb, vl);
				}
			}
			for(int j=0; j<num; j++){
